...
- Fixed working plan exception persistence issue where saving a provider with working plan exceptions a second time would fail with "The provided working plan exception ID does not exist in the database" error; the code was deleting all existing exceptions before saving new ones, causing validation errors when exception IDs were reused. Now exceptions are intelligently updated (preserving IDs) or inserted (creating new IDs), with only truly deleted exceptions being removed from the database
