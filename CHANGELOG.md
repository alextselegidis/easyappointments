...
- Added an opt-in local testing escape hatch for CalDAV SSRF hardening via `Config::ALLOW_PRIVATE_CALDAV_HOSTS` (default: `false`) so private Docker/LAN CalDAV endpoints can be allowed explicitly in development environments
